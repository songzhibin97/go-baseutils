...
func generateStreamNumber(n int) (ret []int) {
	for i := 0; i < n; i++ {
		ret = append(ret, i)
	}
	return ret
...
func TestTaskWhile(t *testing.T) {
	c := Stream(context.Background(), generateStreamNumber(10)...)
	taskWhile := TaskWhile(context.Background(), c, func(v int) bool {
		return v == 5
	})
	var ret []int
	...
	defer cancel()
	c = Stream(context.Background(), generateStreamNumber(10)...)
	taskWhile = TaskWhile(ctx, c, func(v int) bool {
		return v == 5
	})
	ret = nil
	for v := range taskWhile {
...
func TestSkipWhile(t *testing.T) {
	c := Stream(context.Background(), generateStreamNumber(10)...)
	skipWhile := SkipWhile(context.Background(), c, func(v int) bool {
		return v == 0
	})
	var ret []int
	...
	defer cancel()
	c = Stream(context.Background(), generateStreamNumber(10)...)
	skipWhile = SkipWhile(ctx, c, func(v int) bool {
		return v == 0
	})
	ret = nil
	for v := range skipWhile {
