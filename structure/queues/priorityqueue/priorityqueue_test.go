...
func byPriority(a, b Element) int {
	return -bcomparator.IntComparator()(a.priority, b.priority)
}
...
func TestBinaryQueueEnqueueBulk(t *testing.T) {
	queue := NewWith(bcomparator.IntComparator())
...
func TestBinaryQueueIteratorLast(t *testing.T) {
	tree := NewWith(bcomparator.IntComparator())
	it := tree.Iterator()
...
func TestBinaryQueueIteratorNextTo(t *testing.T) {
	...
	// NextTo (empty)
	{
		tree := NewWith(bcomparator.StringComparator())
		it := tree.Iterator()
		for it.NextTo(seek) {
...
func TestBinaryQueueSerialization(t *testing.T) {
	queue := NewWith(bcomparator.StringComparator())
...
func TestBTreeString(t *testing.T) {
	c := NewWith(bcomparator.IntComparator())
	c.Enqueue(1)
