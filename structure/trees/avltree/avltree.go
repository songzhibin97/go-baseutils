...
var _ trees.Tree[any] = new(Tree[any, any])
...
func NewWith[K, V any](comparator bcomparator.Comparator[K]) *Tree[K, V] {
...
func NewWithIntComparator[V any]() *Tree[int, V] {
	return &Tree[int, V]{Comparator: bcomparator.IntComparator()}
}
...
func NewWithStringComparator[V any]() *Tree[string, V] {
	return &Tree[string, V]{Comparator: bcomparator.StringComparator()}
}
...
func removeMin[K, V any](qp **Node[K, V], minKey *K, minVal *V) bool {
	q := *qp
	if q.Children[0] == nil {
		*minKey = q.Key
		*minVal = q.Value
...
func putFix[K, V any](c int8, t **Node[K, V]) bool {
	s := *t
	if s.b == 0 {
		s.b = c
		return true
...
func removeFix[K, V any](c int8, t **Node[K, V]) bool {
	s := *t
	if s.b == 0 {
		s.b = c
		return false
...
func singlerot[K, V any](c int8, s *Node[K, V]) *Node[K, V] {
	s.b = 0
	s = rotate(c, s)
	s.b = 0
	return s
...
func doublerot[K, V any](c int8, s *Node[K, V]) *Node[K, V] {
	a := (c + 1) / 2
	r := s.Children[a]
...
func rotate[K, V any](c int8, s *Node[K, V]) *Node[K, V] {
	a := (c + 1) / 2
	r := s.Children[a]
...
func output[K, V any](node *Node[K, V], prefix string, isTail bool, builder *strings.Builder) {
	if node.Children[1] != nil {
		...
		output(node.Children[1], newPrefix, false, builder)
	}
	...
	if node.Children[0] != nil {
		...
		output(node.Children[0], newPrefix, true, builder)
	}
}
...
func (tree *Tree[K, V]) UnmarshalJSON(data []byte) error {
	...
	if err == nil {
		...
		for key, value := range elements {
			var nk K
			err = tree.Comparator.Unmarshal([]byte(key), &nk)
			if err != nil {
				return err
			}
			tree.Put(nk, value)
...
func (tree *Tree[K, V]) MarshalJSON() ([]byte, error) {
	...
	it := tree.Iterator()
	for it.Next() {
		k, err := tree.Comparator.Marshal(it.Key())
		if err != nil {
			return nil, err
		}
		elements[string(k)] = it.Value()
	}
