...
func TestBTreePut4(t *testing.T) {
	tree := NewWithIntComparator[any](3)
	assertValidTree(t, tree, 0)
...
func TestBTreeIteratorValuesAndKeys(t *testing.T) {
	...
	if actualValue, expectedValue := tree.Values(), []string{"x", "b", "c", "d", "e", "f", "g"}; !reflect.DeepEqual(actualValue, expectedValue) {
		t.Errorf("Got %v expected %v", actualValue, expectedValue)
	}
	if actualValue := tree.Size(); actualValue != 7 {
		t.Errorf("Got %v expected %v", actualValue, 7)
...
func assertValidTreeNode[K, V any](t *testing.T, node *Node[K, V], expectedEntries int, expectedChildren int, keys []int, hasParent bool) {
...
func TestBTreeString(t *testing.T) {
	c := NewWithStringComparator[int](3)
	c.Put("a", 1)
...
func BenchmarkBTreeGet100(b *testing.B) {
	b.StopTimer()
	size := 100
	tree := NewWithIntComparator[struct{}](128)
	for n := 0; n < size; n++ {
		tree.Put(n, struct{}{})
	}
	b.StartTimer()
