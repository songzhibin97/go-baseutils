...
func TestSkipMap(t *testing.T) {
	m := New[int64, any](bcomparator.Int64Comparator())
	...
	m.Range(func(key int64, _ interface{}) bool {
		if key == 123 {
			m.Store(123, 123)
		} else if key == 456 {
			m.LoadAndDelete(456)
	...
	var count int64
	go func() {
		m.Range(func(_ int64, _ interface{}) bool {
	...
	// Correctness 2.
	var m1 sync.Map
	m2 := New[uint32, any](bcomparator.Uint32Comparator())
	var v1, v2 interface{}
	var ok1, ok2 bool
	for i := 0; i < 100000; i++ {
	...
	mp := New[int, any](bcomparator.IntComparator())
	tmpmap := New[int64, any](bcomparator.Int64Comparator())
	samekey := 123
	var added int64
	for i := 1; i < 1000; i++ {
		wg.Add(1)
...
func TestSkipMapDesc(t *testing.T) {
	m := New[int, any](bcomparator.ReverseComparator(bcomparator.IntComparator()))
	cases := []int{10, 11, 12}
	for _, v := range cases {
		m.Store(v, nil)
	}
