...
func NewSafeWith[K, V any](comparator bcomparator.Comparator[K]) *MapSafe[K, V] {
	return &MapSafe[K, V]{
		unsafe: NewWith[K, V](comparator),
	}
}

func NewSafeWithIntComparator[V any]() *MapSafe[int, V] {
	return &MapSafe[int, V]{
		unsafe: NewWithIntComparator[V](),
	}
}

func NewSafeWithStringComparator[V any]() *MapSafe[string, V] {
	return &MapSafe[string, V]{
...
func (s *MapSafe[K, V]) Min() (K, V) {
	s.lock.Lock()
	defer s.lock.Unlock()
	return s.unsafe.Min()
}

func (s *MapSafe[K, V]) Max() (K, V) {
	s.lock.Lock()
	defer s.lock.Unlock()
	return s.unsafe.Max()
}

func (s *MapSafe[K, V]) Floor(key K) (K, V) {
	s.lock.Lock()
	defer s.lock.Unlock()
	return s.unsafe.Floor(key)
}

func (s *MapSafe[K, V]) Ceiling(key K) (K, V) {
	s.lock.Lock()
	defer s.lock.Unlock()
